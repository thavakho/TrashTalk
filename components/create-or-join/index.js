...
const CreateOrJoin = props => {
  ...
  //check for an existing room with the generated code in Firestore
  const checkIfRoomExists = generatedCode => {
    firestore()
      ...
      .then(doc => {
        if (doc.exists) {
          //a room with that code already exists. generate a new code
          generateCode();
        } else {
          //a room with that code does not exist, so create it
