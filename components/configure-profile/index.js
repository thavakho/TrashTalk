  ...
  const getRandomPhoto = (searchTerm) => {
    ...
    .then(data => {
      if (data.query.allimages.length <= 0) {
        ...
        //we should only use jpg or png files so we can be sure they'll display correctly.
        //loop through the results and choose a picture that is a jpg or png
        for (var x=0;x<data.query.allimages.length;x++) {
          ...
          if ((pic.indexOf('.jpg') > 0) || (pic.indexOf('.jpeg') > 0) || (pic.indexOf('.png') > 0)) {
            //we found a good picture, so break out of the loop.
            break;
          } else {
            pic = "";
          }
        ...
        if (pic === "") {
          //we didn't find a good pic, so just use a default
          pic = "https://upload.wikimedia.org/wikipedia/commons/6/69/June_odd-eyed-cat_cropped.jpg";
        }

        //update the user's profile with the picture and name
